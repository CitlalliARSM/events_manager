...
function apphp_db_install($sql_dump_file) {
	...
	// -------------------------------------------------------------
	if(EI_INSTALL_STORED_PROCEDURES){
		$query = 'DROP FUNCTION IF EXISTS `apphp_text_encode`';
		if(!@mysqli_query($link, $query)) $error_mg[] = mysqli_error($link);
	
		$query = 'CREATE FUNCTION  `apphp_text_encode` (x VARCHAR(1024)) 
		...
		$query = 'DROP FUNCTION IF EXISTS `apphp_text_encode_overlib`';
		if(!@mysqli_query($link, $query)) $error_mg[] = mysqli_error($link);
	
		$query = 'CREATE FUNCTION  `apphp_text_encode_overlib` (x VARCHAR(1024)) 
		RETURNS varchar(1024) CHARSET utf8
